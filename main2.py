...
def visualize(testresult, Xdata_t):
    ...
    overlay = np.zeros(
        (Xdata.shape[0], Xdata.shape[1], Xdata.shape[2]), 'uint8')
    overlay[:, :, 0] = layer1
    overlay[:, :, 1] = layer2
    overlay[:, :, 2] = layer3
    im2 = Image.fromarray(overlay, 'RGB')
    im2.save(
        "/Users/wuwenjun/GitHub/CSE-577-Melanoma-Nuclei-Segmentation/output_overlay.jpeg")
    return (im, im2)
...
print('X_train size before normalization: %d %d' % (X_train.shape[0],
                                                    X_train.shape[1]))
scaler = StandardScaler()
X_train = scaler.fit_transform(X_train)
print('X_train size after normalization: %d %d' %
...
filename = 'classifier/modelwithPixelandStruct_NoPCA.sav'
